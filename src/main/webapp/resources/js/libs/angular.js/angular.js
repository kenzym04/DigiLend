 ...
 */
  ...
  function stripWhitespace(str) {
    return isString(str)
      ? str.replace(START_SPACE, '').replace(END_SPACE, '')
      : str
  }
 ...
 */
  ...
  function assertArg(arg, name, reason) {
    if (!arg) {
      throw new Error(
        "Argument '" + (name || '?') + "' is " + (reason || 'required')
      )
    }
    return arg
  }
          ...
          function animateActionFactory(type, beforeFn, afterFn) {
            return function (element, parent, after) {
              ...
              if (!className) {
                ...
                if (
                  (!$sniffer.transitions && !polyfillSetup && !polyfillStart) ||
                ...
                if (polyfillStart) {
                  ...
                  forEach(element, function (element) {
                    if (element.nodeType == ELEMENT_NODE) {
                      ...
                      if (
                        parseFloat(
                          elementStyles[w3cAnimationProp + durationKey]
                        ) > 0 ||
                        parseFloat(
                          elementStyles[vendorAnimationProp + durationKey]
                        ) > 0
                      ) {
                        ...
                        iterations = Math.max(
                          parseInt(
                            elementStyles[w3cProp + animationIterationCountKey]
                          ) || 0,
                          parseInt(
                            elementStyles[
                              vendorProp + animationIterationCountKey
                            ]
                          ) || 0,
                          iterations
                        )
                      ...
                      var parsedDelay = Math.max(
                        parseMaxTime(elementStyles[w3cProp + delayKey]),
                        parseMaxTime(elementStyles[vendorProp + delayKey])
                      )

                      var parsedDuration = Math.max(
                        parseMaxTime(elementStyles[w3cProp + durationKey]),
                        parseMaxTime(elementStyles[vendorProp + durationKey])
                      )

                      duration = Math.max(
                        parsedDelay + iterations * parsedDuration,
                        duration
                      )
                    }
                  })
    ...
    this.$get = [
      ...
      function (
        ...
        function compile($compileNodes, transcludeFn, maxPriority) {
          ...
          forEach($compileNodes, function (node, index) {
            if (
              ...
              $compileNodes[index] = jqLite(node)
                .wrap('<span></span>')
                .parent()[0]
            }
          })
                  ...
                  nodeLinkFn(
                    ...
                    (function (transcludeFn) {
                      return function (cloneFn) {
                        ...
                        return transcludeFn(transcludeScope, cloneFn).bind(
                          '$destroy',
                          bind(transcludeScope, transcludeScope.$destroy)
                        )
                      }
                  ...
                  switch (mode) {
                    case '@': {
                      ...
                      if (attrs[attrName]) {
                        ...
                        scope[scopeName] = $interpolate(attrs[attrName])(
                          parentScope
                        )
                      }
                      break
                      ...
                      scope.$watch(function parentValueWatch() {
                        ...
                        if (parentValue !== scope[scopeName]) {
                          ...
                          if (parentValue !== lastValue) {
                            ...
                            parentSet(
                              parentScope,
                              (parentValue = lastValue = scope[scopeName])
                            )
                          }
                        }
                        return parentValue
        ...
        function addAttrInterpolateDirective(node, directives, value, name) {
          ...
          directives.push({
            priority: 100,
            compile: valueFn(function attrInterpolateLinkFn(
              scope,
              element,
              attr
            ) {
            ...
            fn = function (context) {
              try {
                ...
                var newErr = new Error(
                  'Error while interpolating: ' + text + '\n' + err.toString()
                )
  ...
  function LocationHashbangUrl(appBase, hashPrefix) {
    ...
    this.$$parse = function (url) {
      ...
      if (!isString(withoutBaseUrl)) {
        throw new Error(
          'Invalid url "' + url + '", does not start with "' + appBase + '".'
        )
      }
      var withoutHashUrl =
      ...
      if (!isString(withoutHashUrl)) {
        throw new Error(
          'Invalid url "' + url + '", missing hash prefix "' + hashPrefix + '".'
        )
      }
 ...
 */
  ...
  function $RouteProvider() {
    ...
    this.when = function (path, route) {
      routes[path] = extend(
        { reloadOnSearch: true, caseInsensitiveMatch: false },
        route
      )
     ...
     */
        ...
        function switchRouteMatcher(on, when, whenProperties) {
          ...
          var match = on.match(
            new RegExp(regex, whenProperties.caseInsensitiveMatch ? 'i' : '')
          )
          if (match) {
                ...
                if (next) {
                  ...
                  if (isDefined((template = next.template))) {
                    ...
                    if (isDefined(template)) {
                      ...
                      template = $http
                        .get(template, { cache: $templateCache })
        ...
        function parseRoute() {
          ...
          forEach(routes, function (route, path) {
            if (
              !match &&
              (params = switchRouteMatcher($location.path(), path, route))
            ) {
              match = inherit(route, {
 ...
 */
  ...
  function isSameDomain(requestUrl, locationUrl) {
    ...
    return (
      (domain1.protocol == domain2.protocol || domain1.relativeProtocol) &&
      domain1.host == domain2.host &&
      (domain1.port == domain2.port ||
        (domain1.relativeProtocol &&
      ...
      if (match) {
        ...
        patternValidator = function (value) {
          ...
          if (!patternObj || !patternObj.test) {
            throw new Error(
              'Expected ' + pattern + ' to be a RegExp but was ' + patternObj
            )
          }
      ...
      return {
        ...
        compile: function (element, attr, linker) {
          return function ($scope, $element, $attr) {
            ...
            if (!match) {
              throw Error(
                "Expected ngRepeat in form of '_item_ in _collection_[ track by _id_]' but got '" +
                  expression +
                  "'."
              )
            }
            ...
            $scope.$watchCollection(rhs, function ngRepeatAction(collection) {
              ...
              for (index = 0; index < length; index++) {
                ...
                if (lastBlockMap.hasOwnProperty(trackById)) {
                  ...
                  throw new Error(
                    'Duplicates in a repeater are not allowed. Repeater: ' +
                      expression +
                      ' key: ' +
                      trackById
                  )
      ...
      return {
        ...
        link: function (scope, element, attr) {
          ...
          function update() {
            ...
            if (template) {
              clearContent()
              var enterElements = jqLite('<div></div>')
                .html(template)
                .contents()
    ...
    function ($compile, $parse) {
      ...
      return {
        ...
        link: function (scope, element, attr, ctrls) {
          ...
          // find "null" option
          for (
            var i = 0, children = element.children(), ii = children.length;
            i < ii;
            i++
          ) {
            if (children[i].value == '') {
          ...
          function Options(scope, selectElement, ctrl) {
            ...
            selectElement.bind('change', function () {
              scope.$apply(function () {
                ...
                if (multiple) {
                  value = []
                  for (
                    groupIndex = 0, groupLength = optionGroupsCache.length;
                    ...
                    for (
                      index = 1, length = optionGroup.length;
                      index < length;
                      index++
                    ) {
                      if (
                        (optionElement = optionGroup[index].element)[0].selected
                      ) {
            ...
            function render() {
              ...
              for (index = 0; (length = keys.length), index < length; index++) {
                locals[valueName] =
                  values[keyName ? (locals[keyName] = keys[index]) : index]
                ...
                if (multiple) {
                  selected =
                    selectedSet.remove(
                      trackFn ? trackFn(scope, locals) : valueFn(scope, locals)
                    ) != undefined
                } else {
                  if (trackFn) {
                    var modelCast = {}
                    ...
                    selected =
                      trackFn(scope, modelCast) === trackFn(scope, locals)
                  } else {
                ...
                optionGroup.push({
                  id: trackFn
                    ...
                    : keyName
                    ? keys[index]
                    : index, // either the index into array or key from object
                  label: label,
              ...
              for (
                groupIndex = 0, groupLength = optionGroupNames.length;
                groupIndex < groupLength;
                groupIndex++
              ) {
                ...
                for (
                  index = 0, length = optionGroup.length;
                  index < length;
                  index++
                ) {
